...
class App {
  ...
  setupGeometry() {
    ...
    for(let i = 0; i < 5; i++) {
      ...
      // first triangle
      const vertex1 = [0, 0, 0];
      const vertex2 = [Math.cos(theta) * shortR, Math.sin(theta) * shortR, 0];
      const vertex3 = [Math.cos(halfTheta) * longR, Math.sin(halfTheta) * longR, 0];

      // second triangle
      const vertex4 = [0, 0, 0];
      const vertex5 = [Math.cos(nextTheta) * shortR, Math.sin(nextTheta) * shortR, 0];
      const vertex6 = [Math.cos(halfTheta) * longR, Math.sin(halfTheta) * longR, 0];

      this.position.push(...vertex1, ...vertex2, ...vertex3, ...vertex4, ...vertex5, ...vertex6);

      const colorArray = [
        0.0, 0.9, 0.8, 0.6, // ひとつ目の頂点の r, g, b, a カラー
        0.0, 0.4, 1.2, 1.0, // ふたつ目の頂点の r, g, b, a カラー
        0.4, 0.0, 2.0, 1.0, // みっつ目の頂点の r, g, b, a カラー
